...
with open('profile_pictures/role_colors.json') as file:
...
if __name__ == '__main__':
    ...
    nums_to_skip = input('Please enter file name numbers that are to be skipped, seperated by comma (i.e. 001, 002, 003 etc). Leave empty if none >  ')
    nums_array = nums_to_skip.split(",")  # Split the string by comma and obtain individual values
    nums_array = [num.strip() for num in nums_array] # Remove leading/trailing spaces from each value
        
    current_time = datetime.datetime.now()
    save_images(lines, init_time=current_time, nums_to_skip=nums_array)
    gen_vid(filename)
